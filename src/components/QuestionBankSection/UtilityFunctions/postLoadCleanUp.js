...
export function postLoadCleanUp(questionbank) {
    ...
    for (var i = 0; i < questionbank.length; i++) {
        if (questionbank[i].type === "category") {
            ...
            for (var k = 0; k < cats.length; k++) {
                if (k === 0) dum_cats += cats[k];
                else dum_cats += "/" + cats[k]; //check if there is already a dummy category for this one in the question bank
                if (getDumCatKey(dum_cats, questionbank) === -1) {
                    //if the dummy category is not yet in the question bank, because it's empty, we'll create it
    ...
    if (numcats === 0) {
        //this can happen when you export a single question in Moodle and not the entire questionbank
        var newq = { name: "top", type: "category", category: "top" };
